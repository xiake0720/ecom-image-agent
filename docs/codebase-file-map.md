...
- `src/workflows/nodes/director_v2.py`
  - 图组导演规划，基于高层风格意图自动规划整套图，区分产品参考图和背景风格参考图，并确定各图位 copy 策略与 hero 2/3 占比
- `src/workflows/nodes/prompt_refine_v2.py`
  - 逐图 prompt、自动标题、副标题、卖点、版式、字体层级和是否带字策略收口
...
## Domain
- `src/domain/task.py`
  - 任务高层输入与进度字段，只保留品牌、商品、整体风格、张数、比例、分辨率等最小参数
- `src/domain/asset.py`
  - 上传素材及素材类型，支持 `BACKGROUND_STYLE`
- `src/domain/director_output.py`
  - `director_output.json` 对应 contract，包含整套图策略、各图位卖点方向、copy 策略和 hero 主体比例
- `src/domain/prompt_plan_v2.py`
  - `prompt_plan_v2.json` 对应 contract，包含最终 render prompt、copy_strategy、should_render_text、title/subtitle/selling points/layout/typography
...
- `src/providers/image/runapi_gemini31_image.py`
  - RunAPI Gemini 3.1 图片调用，区分产品参考图和背景风格参考图，并显式下发 copy_strategy / should_render_text
...
- `src/ui/components/upload_panel.py`
  - 外包装白底图、产品补充参考图、背景风格参考图上传区
- `src/ui/pages/task_form.py`
  - 极简高层输入区，只保留品牌、商品、整体风格和高级设置
