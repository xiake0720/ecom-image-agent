...
- `/login`
  - 前端壳层页面已保留。
  - 后端真实认证 API 已补齐，但前端尚未接入。
- `/register`
  - 前端壳层页面已保留。
  - 后端真实注册 API 已补齐，但前端尚未接入。
- `/main-images`
  - 正式主图工作台。
- `/detail-pages`
  - 正式详情图工作台。
- `/tasks`
  - 正式历史任务记录页。
  - 当前支持回看 `main_image` 和 `detail_page_v2`。
...
## 8. 后续改造顺序
1. 先把前端 `/login`、`/register` 接到 `/api/v1/auth/*`，补最小登录态守卫和退出流程。
2. 再把“局部标记后二次生成”定义为正式任务类型、正式 API 和正式回看链路。
3. 然后把数据库里的 `tasks`、`task_events`、`task_results` 与现有 JSON 任务链路逐步对齐。
